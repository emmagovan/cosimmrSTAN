...
#' 'prop_histogram', 'prop_density', 'beta_fixed_density', 'beta_fixed_boxplot',
#' 'beta_random_density', 'beta_random_boxplot'
...
#'
#' @return one or more of 'isospace', 'beta_fixed_density',
#' 'beta_fixed_boxplot', 'prop_histogram', 'prop_density', 'beta_random_density'
...
plot.cosimmrSTAN_output <-
  function(x,
    ...
    if (inherits(x, "cosimmrSTAN_output") == TRUE) {
      ...
      if (is.null(x$output$beta_random) & (("beta_random_density" %in% type) | ("beta_random_boxplot" %in% type) | ("omega_density" %in% type))) {
...
model or plots to create and rerun.")
      } else if (n_fixed_cov == 0 & (("beta_fixed_density" %in% type) | ("beta_fixed_boxplot" %in% type))) {
...
model or plots to create and rerun.")
        ...
        for (i in 1:length(obs)) {
          if (is.null(title_input) == TRUE) {
            ...
            title <- rep(title_input, length(obs))
          }
          curr_ind <- obs[i]
        ...
        if ("beta_fixed_density" %in% type) {
          ...
          for (l in 1:n_fixed_cov) {
            ...
            if (is.null(title_input) == TRUE) {
              title <- paste("beta density plot for", c_name, "covariate")
            } else {
              title <- title_input
            }
        ...
        if ("beta_random_density" %in% type) {
          for (l in 1:total_levels) {
            ...
            if (is.null(title_input) == TRUE) {
              title <- paste("beta density plot for", c_name, "covariate, level", level_name)
            } else {
              title <- title_input
            }
            ...
            if (one_plot == TRUE) {
              ...
              if (box_or_line == "LINE") {
                ...
                g <- ggplot(data = df_plot, aes(x = cov, y = mean, colour = Source)) +
                  ...
                  geom_line() +
                  theme_bw() +
                  xlab(colnames(cov_selected_col)) +
                  ylab(paste("Proportion (\u00B1 2sd)")) +
                  ggtitle(paste0("Change in consumption over ", colnames(cov_selected_col)))
              ...
              for (s in 1:source_loop) {
                ...
                if (box_or_line == "LINE") {
                  ...
                  g <- ggplot(data = df_plot, aes(x = cov, y = mean)) +
                    ...
                    xlab(paste0(colnames(cov_selected_col))) +
                    ylab(paste("Proportion (\u00B1 2sd)"))
        ...
        if("omega_density" %in% type){

          if (is.null(title_input) == TRUE) {
              title <- paste("Omega density plot")

          } else {
            title <- title_input
          }
          omega = x$output$omega #This is n_samples * n_sources




          colnames(omega) <- x$input$source_names

          df <- reshape2::melt(omega)


          colnames(df) <- c("Num", "Source", "Omega")
